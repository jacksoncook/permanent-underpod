"""Perp of Fortune v3: GLM 4.7 Flash reads the episode prep guide (perp section
removed) + does live Hyperliquid research, then picks a news-informed perp."""
import json
import time
...
GUIDE = open("/private/tmp/claude-501/-Users-jcook-Personal-permanent-underpod/c9b9487b-ce5a-424f-84b6-f174f2f2b185/scratchpad/guide-for-glm.md").read()
...
def market_overview():
    ...
    out = ["ALL LIVE TICKERS: " + ", ".join(sorted(COINS)), "\nTOP 10 BY 24H VOLUME:"]
...
SYSTEM = f"""You are this week's guest picker on "Perp of Fortune," a recurring bit on a Bitcoin/stablecoins/AI podcast. The hosts run a tiny real-money entertainment account on Hyperliquid; an AI picks one leveraged perp each episode.

You are GLM, running locally as open weights on the host's machine.

Below is the episode's actual prep guide — this week's researched news. Use it: your pick should connect to a real story in the guide, then be sanity-checked against live market data with the tools.

=== EPISODE PREP GUIDE (news research) ===
{GUIDE}
=== END PREP GUIDE ===

Tools — to call one, reply with EXACTLY one line per call (multiple lines allowed):
TOOL: market_overview
TOOL: coin_stats <TICKER>

Rules:
- Up to 5 research rounds. Use them — check the live data before picking.
- BANNED picks: BTC (house rule) and PUMP (position still open).
- When ready, give your final answer in EXACTLY this format:
PICK: <ticker>
DIRECTION: <long|short>
LEVERAGE: <number>x
NEWS THESIS: <2-3 sentences tying the pick to a specific story in the prep guide>
RESEARCH BASIS: <1-2 sentences citing the live numbers you pulled>
REASONING: <2-3 sentences, entertainment-grade, in your own voice>
...
def chat(messages):
    ...
    msg = json.loads(urllib.request.urlopen(req, timeout=900).read())["message"]
    return msg.get("thinking") or "", msg.get("content") or ""


messages = [{"role": "system", "content": SYSTEM},
            {"role": "user", "content": "The board is live. Read the guide, do your research, then pick."}]
transcript = []
...
with open("/Users/jcook/Personal/permanent-underpod/episodes/ep11/perp-pick-glm-v3-raw.json", "w") as f:
    json.dump(transcript, f, indent=2)
print("\ntranscript saved: episodes/ep11/perp-pick-glm-v3-raw.json", flush=True)
