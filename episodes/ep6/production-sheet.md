...
**Main:** We Let the AI Bet Again — It Picked Solana's MEV Cartel

Alternates:
- The $15M Sandwich Bot Just Got Sandwiched
- Our AI Clones Hosted the Show Again (They're Getting Better)
- Fable vs GPT-5.6: We Made Them Do the Same Job
- Claude Goes 5x Long and Finally Prints
...
1. Episode first via `yt_upload.py` (scheduled-private with a `publishAt`).
2. Backfill `episodes/ep6/segment-times.md` from the LIVE copy (`yt_fetch.py`).
3. Then clips (clipify), staggered daily 2 PM PT; work the funnel checklist.
